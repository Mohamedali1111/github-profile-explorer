...
export default function RepoCard({ repo }: RepoCardProps) {
	...
	return (
		<div className="group rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/70 backdrop-blur-sm p-5 shadow-sm hover:shadow-md transition-all cursor-pointer">
			<div className="flex items-start justify-between gap-3">
				<div className="min-w-0">
					<Link href={repo.html_url} target="_blank" className="font-semibold text-gray-900 dark:text-white hover:text-blue-700 dark:hover:text-blue-400 truncate">
						{repo.name}
					</Link>
					{repo.description && (
						<p className="mt-1 text-sm text-gray-600 dark:text-gray-300 line-clamp-2">{repo.description}</p>
					)}
				</div>
			...
			<div className="mt-4 flex items-center gap-4 text-sm text-gray-600 dark:text-gray-300">
				...
				<span className="text-gray-500 dark:text-gray-400">Updated {formattedDate}</span>
			</div>
		</div>
	);
}
